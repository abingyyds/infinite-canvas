...
import { createVideoGenerationTask, pollVideoGenerationTask, storeGeneratedVideo, VIDEO_TASK_TIMEOUT_MS, type VideoGenerationTask } from "@/services/api/video";
...
export default function VideoPage() {
    ...
    const generate = async () => {
        const snapshot = await buildRequestSnapshot();
        if (!snapshot) return;
        setElapsedMs(0);
        setRunning(true);
        setPreviewLog(null);
    ...
    const buildRequestSnapshot = async () => {
        ...
        const hydratedReferences = await hydrateReferenceImages(references);
        setReferences(hydratedReferences);
        return { text, config: buildVideoConfig(effectiveConfig, model), references: hydratedReferences, videoReferences: [...videoReferences], audioReferences: [...audioReferences] };
    };
    ...
    const previewGenerationLog = (log: GenerationLog) => {
        void (async () => {
            const hydratedReferences = await hydrateReferenceImages(log.references || []);
            setPreviewLog(log);
            setLogsOpen(false);
                                    ...
                                    {references.map((item, index) => (
                                        <div key={item.id} className="group relative size-20 shrink-0 overflow-hidden rounded-md border border-stone-200 dark:border-stone-800">
                                            {item.dataUrl ? (
                                                ...
                                                <div className="flex size-full items-center justify-center bg-stone-100 px-1 text-center text-[10px] leading-4 text-stone-500 dark:bg-stone-900 dark:text-stone-400">图片已失效</div>
                                            )}
...
function serializeLog(log: GenerationLog): GenerationLog {
    return {
        ...log,
        references: log.references.map((item) => (item.storageKey ? { ...item, dataUrl: "" } : { ...item, dataUrl: item.dataUrl || "" })),
...
async function hydrateReferenceImages(items: ReferenceImage[]) {
    return Promise.all(
        items.map(async (item) => ({
            ...item,
            dataUrl: await resolveImageUrl(item.storageKey, item.dataUrl || item.url || ""),
        })),
    );
}
