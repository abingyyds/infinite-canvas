...
import { computeMediaSize, parseAspectRatio } from "@/lib/media-size";
            ...
            merge: (persisted, current) => {
                ...
                return {
                    ...
                    config: {
                        ...
                        size: migrateImageSize(config, (persistedConfig as { resolution?: string }).resolution),
...
/** Older builds kept the 1k/2k/4k tier in a separate `resolution` field; fold it into `size`. */
function migrateImageSize(config: AiConfig, legacyResolution: string | undefined) {
    if (!legacyResolution || legacyResolution === "auto" || !parseAspectRatio(config.size)) return config.size;
    return computeMediaSize(legacyResolution, config.size) || config.size;
}
