...
+ [修复] 直连网关生成视频时，结果地址与配置的 Base URL 域名不同也会带 Key 下载，不再把鉴权失败的响应当成视频保存，导致无法预览和下载。
...
+ [修复] 模型账号登录支持 Turnstile 与双重验证，并强化模型选择入口和自动密钥的官方通道配置。
+ [修复] Seedance 视频改走网关的 /videos 端点，修复 model not found 报错。
+ [调整] 移除后端远程提示词源抓取与定时同步，站内提示词库只保留后台维护的内容，远程来源统一由前端 registry 提供。
+ [调整] 提示词来源与封面对齐上游：移除站内提示词库内置来源和封面代理，封面直连图床。
+ [修复] gpt-image 系列不再发送 response_format 与 output_format，修复 invalid_request 报错。
+ [修复] 网关的 veo、omni 视频模型改用 JSON 版 /videos 请求体，修复参数被拒。
