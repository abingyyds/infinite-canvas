...
- Model-account login with Turnstile and two-factor authentication, including pending-session verification, merged cookies, official-channel key creation for main and distributor accounts, the optional 2FA field, and explicit current-model or model-selection labels.
